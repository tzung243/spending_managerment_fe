...
const EditProfile = (props) => {
  ...
  const { register, handleSubmit, errors, setValue, setError, control } =
    useForm({
  ...
  const requestCurrencies = useCallback(async () => {
    if (state.currencies.length === 0) {
      await axios
        .get(currencyApiEndpoints.currency, {})
        .then((response) => {
  ...
  const currencyTemplate = (option) => {
    return (
      <span>
        <span className="color-highlight text-bold">
      ...
      .then((response) => {
        ...
        if (response.status === 200) {
          ...
          setValue([
            ...
            {
              currency: state.currencies.find((el) =>
                el.id === response.data.request.currency_id ? el : null
              ),
            },
          ]);
          ...
          <Card className="rounded-border">
            <div>
              ...
              <div className="p-card-subtitle">
                Edit current profile information below.
              </div>
            </div>
            <br />
